...
import {
  matchTailwindClassesHeight,
  matchTailwindClassesWidth,
  modifyLayoutClassesChild,
} from "@/app/(main)/(editor)/utils/tailwind";
import { PreviewComponent } from "@/app/canvas/preview-component";
import {
  Card,
  CardHeader,
  CardTitle,
...
const componentMap = {
  ...
  image: Image,
  link: Link,
  previewcomponentfinal: PreviewComponentFinal,
  icon: Icon,
  code: "code",
  pre: "pre",
  dialog: Dialog,
...
export const renderComponent = (
  ...
  let { parentClasses, childClasses } = modifyLayoutClassesChild(
    item?.props?.className ?? ""
  );
  if (dontWidth.includes(item?.type?.toLowerCase() ?? '')) {
    parentClasses?.map((classT: string) => {
      childClasses.push(classT)
    })
    // childClasses = {...childClasses, ...parentClasses}
  }
...
// window.localStorage.setItem('traqix_mem_ddddd', dataSave);
  ...
  if (matchesWidth.length) {
    ...
    if (!add && !dontWidth.includes(item?.type?.toLowerCase() ?? '')) {
      childClasses.push("w-full");
    }
  }

  if (item?.type.toLowerCase() === 'avatarfallback') {
    // item.props.className = `${item.props?.className ?? ''} w-full`;
    // childClasses.push("w-full");
    // newStyle = {
    //   ...newStyle,
    //   width: "100%",
  ...
  const matchesHeight = matchTailwindClassesHeight(item?.props?.className ?? "");
  if (!matchesHeight.length) {
    childClasses.push("h-full");
  }
  ...
  const commonProps: any = {
    ...
    className: `${childClasses.join(" ") || ""} ${
      childClasses.includes("container") || parentClasses.includes("mx-auto")
        ? "mx-auto"
        : ""
  ...
  switch (item?.type.toLowerCase()) {
    ...
    case "popovertrigger":
      const DynamicComponentSpecial1 = componentMap[item?.type?.toLowerCase()] || 'div';
      return (
    ...
    case "button":
      return (
        <Button {...commonProps} ref={ref} onClick={(e) => handleClick(e, item?.props?.onclick)}>
          {item?.props?.content ? item?.props?.content : ''}
          {item.children &&
            item.children.map((childId) => {
              return (
                <PreviewComponent
    ...
    case "tablerow":
    case "tablecell":
      const DynamicComponentSpecial = componentMap[item?.type?.toLowerCase()] || 'div';
      return (
    ...
    // defaults no children

    case "avatarimage":
      const DynamicComponentNoChildren = componentMap[item?.type?.toLowerCase()] || item?.type?.toLowerCase();

      return <DynamicComponentNoChildren {...commonProps} ref={ref} />;
    ...
    case "section":
    default:
      const DynamicComponent = componentMap[item?.type?.toLowerCase()] || item?.type?.toLowerCase();
      return (
